...
const DtLineChartMultiView = (props) => {
  ...
  useEffect(() => {
    ...
    setCheckedVal(0);
    appendData(mChartDetails, 'Following', filename, 'Adopters\' Median "Following" Over Time');
      ...
      options.push(
        <label key={col}>
          <input
            name="radio"
            type="radio"
            onChange={() => { appendData(chartDetails, col, filename, `Adopters' Median "${col}" Over Time`); setCheckedVal(i); }}
